...
async function getBase64ImageUrl(
  ...
  try {
    ...
    console.error(`Error generating blur data for ${imageId}:`, error);
    return undefined;
  }
}
...
function sortResourcesByDateDesc(resources: CloudinaryResource[]): CloudinaryResource[] {
  return [...resources].sort((a, b) => {
    ...
    return dateDelta !== 0 ? dateDelta : b.public_id.localeCompare(a.public_id);
  });
}
...
export async function getPhotos(withBlur = false): Promise<PhotoProps[]> {
  ...
  try {
    ...
    const resourcesToBlur = withBlur ? resources.slice(0, BLUR_DATA_LIMIT) : [];
    const imagesWithBlurData = await Promise.all(
      resourcesToBlur.map((result) => getBase64ImageUrl(result.public_id, result.format))
    );
    ...
    return resources.map((result, index) => ({
      ...
      blurDataUrl: withBlur && index < BLUR_DATA_LIMIT
        ? imagesWithBlurData[index]
        : undefined,
    }));
  } catch (error) {
...
export async function getPhotoByRouteParam(routeParam: PhotoRouteParam): Promise<PhotoProps | null> {
  ...
  if (isLegacyPhotoIndexParam(photoId)) {
    ...
    return numericIndex < 0 ? null : photos[numericIndex] ?? null;
  }

  const matchedPhoto = findPhotoByRouteParam(routeParam, photos);
  return matchedPhoto ?? getPhotoById(photoId);
}
