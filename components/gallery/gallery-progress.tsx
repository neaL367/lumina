...
export function GalleryProgress() {
  ...
  return (
    <div className="fixed bottom-8 left-8 right-8 z-40 flex items-center justify-center text-zinc-500 dark:text-zinc-400 font-mono text-xs select-none pointer-events-none" style={{ bottom: "calc(2rem + env(safe-area-inset-bottom))" }}>
      <div className="flex items-center gap-4 pointer-events-auto" style={{ viewTransitionName: "gallery-progress" }} role="img" aria-label={`Memory ${Math.round(p) + 1} of ${items.length}`}>
        ...
        <div className="relative w-20 sm:w-28 h-[2px] bg-zinc-300/50 dark:bg-zinc-800/55 rounded-full" aria-hidden="true">
          <div
