...
export default function InstallPrompt(): React.JSX.Element | null {
  const isIOS = useSyncExternalStore(subscribeNever, getIsIOS, getServerSnapshot);
    ...
    <div className="fixed bottom-4 left-4 right-4 z-9999 bg-black/80 backdrop-blur-md border border-white/10 rounded-xl p-4 flex items-start justify-between text-white/90 shadow-2xl" style={{ viewTransitionName: "install-prompt" }}>
      <div>
        ...
        <p className="text-sm text-white/70">
          ...
          <span role="img" aria-label="share icon" className="text-white">
            ⎋
          </span>{" "}
          and then <strong>Add to Home Screen</strong>{" "}
          <span role="img" aria-label="plus icon" className="text-white">
            ➕
          </span>
          .
        </p>
